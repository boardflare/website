---
title: Functions and aligning with Python in Excel
date: 2024-10-22
---

# Functions and aligning with Python in Excel
October 22, 2024
...
One of the great features of `BOARDFLARE.RUNPY` is that it can be used to build LAMBDA functions that can be used in Excel formulas.  For example, the `text_distance` function can be used to build a `FUZZY.DIST.MATCH` function that is similar to Excel's `XMATCH` function, but with the added ability to set a similarity threshold.  Here's the code:
...
Or, a `FUZZY.DIST.LOOKUP` function can be built that is similar to Excel's `XLOOKUP` function, but with the added ability to return a row from a `return_array`.  Here's the code:
