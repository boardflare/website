...
Some things to about arguments are as follows:
- Global variables are cleared after each function invocation.
- Arrays are converted to a pandas DataFrame without headers.
- Skipping args with a comma, sets the skipped arg to None. e.g. `=BOARDFLARE.RUNPY(A1,,3)` sets `arg1 = None` and `arg2 = 3`.
- Unset optional args in a LAMBDA become `None`. e.g. `=LAMBDA([x], BOARDFLARE.RUNPY("3 + arg1", x))()` will set `arg1 = None` and throw an error because `None` is not a valid Python type for the `+` operator, so you would need to handle this case in your Python code.
