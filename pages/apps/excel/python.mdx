...
Network access is available to the Pyodide runtime via JavaScript. There are several options available using this.
1. Directly using [pyfetch](https://pyodide.org/en/stable/usage/api/python-api/http.html#pyodide.http.pyfetch) - This is a Python wrapper around the Fetch API provided by Pyodide. This can be used to make HTTP requests to APIs.
...
```
2. Patching http libraries with [pyodide-http](https://github.com/koenvo/pyodide-http) - This patches http libraries such as `requests` so that they use JavaScript for network access.

```python
  # Patch requests to use JavaScript for network access.
  import pyodide_http
  pyodide_http.patch_all()

  import requests
  response = requests.get("https://some_url/myfiles.zip")
```
