...
Provides unlimited free and secure AI functions by using a small language model, currently [Gemma2 2B](https://github.com/google-deepmind/gemma), that runs locally on your computer so your data never leaves Excel.  While not as fast and smart as ChatGPT, maybe it is all you need for simple tasks.  If you have access to an advanced GPU on your computer, it may be possible to run much larger and more powerful models. Please [let us know](/company/support) if you are interested in this as a paid upgrade.
...
🆓 Unlimited free use of small models.<br/>
...
These are the initial functions provided.  LOCAL.GPT is the most versatile and enables you to prompt the model to do a wide range of tasks.  The other functions are more specialized and are provided for convenience.

### LOCAL.GPT

=<code>BOARDFLARE.LOCAL.GPT (prompt, [data], [options])</code><br/>
...
=<code>BOARDFLARE.LOCAL.CLASSIFY (text, categories)</code><br/>
...
<code>=BOARDFLARE.LOCAL.CLASSIFY(A1, B1:B5)</code><br/>


### LOCAL.SUMMARIZE

=<code>BOARDFLARE.LOCAL.SUMMARIZE (text)</code><br/>
<code>text:</code> Text to summarize.<br/>
E.g., to summarize the content of cell A1:<br/>
<code>=BOARDFLARE.LOCAL.SUMMARIZE(A1)</code><br/>

This just uses the LOCAL.GPT function with a prompt of "Summarize the following text:".  It is provided for convenience.

### LOCAL.TRANSLATE

=<code>BOARDFLARE.LOCAL.TRANSLATE (text, source_lang, target_lang, [model])</code><br/>
...
<code>=BOARDFLARE.LOCAL.TRANSLATE(A1, "fr", "en")</code><br/>
